...
public class UUIDGenerator {
	...
	public static String generateMsgId(String exchCode){
		return new StringBuilder(32).append(formatDate(new Date()))
				.append(exchCode).append(format(getJVM())).append(format(getCount())).toString();
	}
	...
	{
		StringBuffer shortBuffer = new StringBuffer("8");
		...
		for (int i = 0; i < 11; i++)
		{
			...
			if (i == 10)
			{
				str = uuid.substring(i * 3, i * 3 + 2);
			}
			else
			{
				str = uuid.substring(i * 3, i * 3 + 3);
			}
